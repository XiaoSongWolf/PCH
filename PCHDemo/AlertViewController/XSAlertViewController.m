...
#import "XSAlertViewController.h"
...
@interface XSHighLightButton : UIButton

@property (strong, nonatomic) UIColor *highlightedColor;

@end

@implementation XSHighLightButton
...
@interface XSAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(XSAlertAction *action);

@end

@implementation XSAlertAction

+ (instancetype)actionWithTitle:(NSString *)title titleColor:(NSInteger)titleColor handler:(void (^)(XSAlertAction *action))handler {
    XSAlertAction *instance = [XSAlertAction new];
...
@interface XSAlertViewController () {
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation XSAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    XSAlertViewController *instance = [XSAlertViewController new];
...
- (void)creatContentView {
    ...
    UIImageView *imageV = [[UIImageView alloc]initWithFrame:CGRectMake((CGRectGetWidth(_contentView.frame)-79)/2.0, CGRectGetMinX(_contentView.frame)-45, 79, 70)];
    imageV.image = [UIImage imageNamed:@"Online_audition"];
...
- (void)creatAllButtons {
    for (int i=0; i<self.actions.count; i++) {
        XSHighLightButton *btn = [XSHighLightButton new];
        btn.tag = 10+i;
...
- (void)updateAllSeparatorLineFrame {
    ...
    // 2个按钮时, 在按钮中间加一个分割线
    if (self.actions.count == 2) {
        UIView *lineView = [_contentView viewWithTag:300];
        lineView.frame = CGRectMake((_contentView.frame.size.width-1)/2.0,CGRectGetMaxY(_contentView.frame)- 40, 1, 35);
    }
}
...
- (void)didClickButton:(UIButton *)sender {
    XSAlertAction *action = self.actions[sender.tag-10];
